...
1. 목표 변수의 오프셋 구하기
   - 방법 1 (CLI) : `nm` 명령어 사용   
     `nm -D [파일명] | grep [찾고_싶은_심볼명]`
     - 출력되는 주소값이 바이너리 시작점으로부터의 오프셋
     - 출력 결과 중간에 있는 문자가 해당 심볼의 성격을 나타냄
       - T / t : 코드 영역(Text section)에 있는 함수
       - D / d : 초기화된 데이터 영역(Data section)
       - B / b : 초기화되지 않은 영역(BSS section)
       - U : 정의되지 않음 (외부 라이브러리에서 가져다 쓰는 경우)
