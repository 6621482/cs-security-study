...
- 인코딩이 필요한 특수문자 : :/?#[]@!$'()*+,;=%공백
- 인코딩 방법 : % 기호 뒤에 해당 문자의 아스키 코드 16진수 값을 붙
- ex. 공백 -> %20
...
- Base64 인코딩 방식
  - (1) 원본 이진 데이터를 비트 나열로 표현하고, 이를 6비트씩 끊어서 묶음(부족하면 뒤에 0 채움)
  - (2) 각 6비트 묶음을 수로 변환한 뒤, base64 테이블에서 해당하는 문자를 찾아 치환
  - (3) 글자 수가 4의 배수가 되도록 문자 '='을 반복해 뒤에 추가 (padding)
    - padding 하는 이유 : 6비트씩 묶어서 인코딩하고, 디코딩 시에는 8비트 단위로 복원해야 하므로 두 단위의 최소공배수인 24비트에 맞추기 위해 결과 글자수를 4의 배수로 만듦
- **Example** : dream
  - dream = d(01100100) r(01110010) e(01100101) a(01100001) m(01101101)
---
| 6-bit | 011001 | 000111 | 001001 | 100101 | 011000 | 010110 | 110100 | 000000 |
|------|--------|--------|--------|--------|--------|--------|--------|--------|
| 10진수 | 25 | 7 | 9 | 37 | 24 | 22 | 52 | 0 (패딩) |
| Base64 문자 | Z | H | J | l | Y | W | 0 | = |

### Operating System
- 응용 프로그램의 동작을 수행하고, 응용 프로그램에게 시스템 자원을 할당하는 등의 복잡한 관리 작업을 하는 소프트웨어
- 사용자, 응용 프로그램 - 운영체제 - 하드웨어(CPU, 메모리, 입출력 장치 등)
- 하드웨어 자원을 효율적으로 관리할 수 있도록 분배, 할당하여 성능을 높임
  - CPU 스케줄링 : 어떤 프로세스에 CPU를 할당할지 결정
  - 메모리 공간을 각 프로세스에 분배하고 사용하는 과정 관리
  - 컴퓨터 입출력 장치와 정보를 주고 받는 과정 관리
- **Kernel** : 하드웨어 관리를 실제로 수행하는 프로그램
  - 소프트웨어와 하드웨어 간의 커뮤니케이션 관리
- **Shell** : 사용자와 운영체제의 커널 사이에서 사용자가 운영체제에 명령을 내릴 수 있도록 인터페이스 역할을 함
  - 사용자가 셸에 명령을 입력하면, 셸이 명령어를 해석하여 커널에 요청
  - 커널은 명령을 수행하며 하드웨어를 조작하고 수행 결과를 셸에 전송
  - 셸은 이 결과를 해석하여 사용자에게 출력
  > 셸을 획득하면 명령어를 통해 원하는 작업을 수행하고 시스템을 제어할 수 있게 됨. 셸 획득 = 시스템 해킹의 성공공
